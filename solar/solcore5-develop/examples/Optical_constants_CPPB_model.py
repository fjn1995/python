...
function of GaAs.读入材料参数，用修正的临界点抛物带模型（CPPB）近似复杂介电常数砷化镓的功能
...
# Material_Params方法以字典变量的形式加载所需的材料参数
MatParams = CPPB_Model.Material_Params("GaAs")

# Parameters can be customised by assigning to the correct dictionary key...参数可以通过分配给正确的字典键来自定义
MatParams["B1"] = 5.8
MatParams["B1s"] = 1.0
...
# Must define a structure object containing the required oscillator functions    必须定义一个包含所需振子函数的结构对象
# The oscillator type and material parameters are both passed to individual 'Oscillators' in the structure...振子类型和材料参数都传递给结构中的单个“振子”
...
# Subplot I :: Real part of the dielectric function.副图一:介电函数的实部。
ax1.set_yscale("linear")        # y轴线性模式
ax1.set_xlim(0, 5.3)
ax1.set_ylim(-14, 27)
