...
"""介电常数"""
# 用锗衬底在79℃的椭圆偏振光谱法计算了复介电函数的实部和虚部∘ (点），以及使用TMM计算的由半无限Ge衬底和4.5 nm GeO2（线）组成的模型的介电常数
E_eV = np.linspace(0.7, 4.2, 1000)

# Load in ellipsomery data from file...
Exp_Data = EllipsometryData("data/ge_ellipsometry_data.dat")        # 椭圆测量术
Exp_Angles = Exp_Data.angles

# Load in some experimental Ge n-k to compare fit with this...
Ge_nk_Exp = np.loadtxt("data/Ge_nk.csv", delimiter=",", unpack=False)       # 读取Ge的n--k
# print(Ge_nk_Exp)
# Smooth the data with spline fitting...用样条拟合平滑数据
# x=Ge_nk_Exp[::5, 0]   # 表示取第0列为该数组,然后在第0列,每第5行   从第一行开始直到最后一行.
# print(x)
n_spline = InterpolatedUnivariateSpline(x=Ge_nk_Exp[::5, 0], y=Ge_nk_Exp[::5, 1], k=3)(E_eV)    #插值方差分析拟合
...
# Define Oscillator Structure 定义振荡器结构
GeO2 = Structure([
...
# Custom_CPPB.nk_calc() 计算折射率和消光系数 从复杂介电函数。
GeO2_nk = cppb().nk_calc(oscillator_structure=GeO2, energy_array=E_eV)

# Step 2 :: use this modelled n and k to calculate the ellipsometry data...用这个模型n和k计算椭圆偏振数据
# Define a structure for the optical stack...定义光堆栈的结构
stack = Structure([
