...
这主要是基于Steven Byrnes的tmm包，通过修改使代码在波长上矢量化(由Diego Alonso-Alvarez完成)，
并包括使用比尔-朗伯定律(由Phoebe Pearce完成)在非相干层中依赖深度的吸收计算。
...
class absorp_analytic_fn:
    ...
    其中a(z)是在深度z处的吸收，z=0是层的开始，
    A1 A2 A1 a3是实数，A1 >0, a3>0, a3是复数。
    类存储这5个参数，以及层厚度d。
    ...
    def copy(self):
        """
        Create copy of an absorp_analytic_fn object         创建一个absorp_analytic_fn对象的副本
        """
    ...
    def scale(self, factor):
        """
        multiplies the absorption at each point by "factor".  将每个点的吸收量乘以“因子”。
        """
        self.A1 *= factor
        self.A2 *= factor
        self.A3 *= factor
    ...
    def add(self, b):
        """
        adds another compatible absorption analytical function 增加另一个兼容的吸收分析功能
        """
